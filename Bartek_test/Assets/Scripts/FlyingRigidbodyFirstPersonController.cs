...
public class FlyingRigidbodyFirstPersonController : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (flying)
		{
			rigidbodyFPC.mouseLook.LookRotation (transform, rigidbodyFPC.transform);
		}
	}
	...
	void FixedUpdate()
	{
		if (flying)
		{
			...
			{
				Vector3 desiredMove = rigidbodyFPC.transform.forward*input.y + rigidbodyFPC.transform.right*input.x;
